...
class DBHandler (context: Context, name: String?, factory: SQLiteDatabase.CursorFactory?, version: Int) :
    ...
    fun getFavourites(context: Context) : ArrayList<Favourites>{
        ...
        if (cursor.count == 0){
            ...
            while(cursor.moveToNext()){
                ...
                favourites.favouritesID = cursor.getInt(cursor.getColumnIndex(COLUMN_FAVOURITE_ID))
                favourites.favouritesNasaID = cursor.getString(cursor.getColumnIndex(COLUMN_FAVOURITE_NASAID))
                favourites.favouritesImageLink = cursor.getString(cursor.getColumnIndex(COLUMN_FAVOURITE_IMAGE_LINK))
                favourites.favouritesTitle = cursor.getString(cursor.getColumnIndex(COLUMN_FAVOURITE_TITLE))
                favourites.favouritesCreator = cursor.getString(cursor.getColumnIndex(COLUMN_FAVOURITE_CREATOR))
                favourites.favouritesDate = cursor.getString(cursor.getColumnIndex(COLUMN_HISTORY_DATE))
                favourites.favouritesDescription = cursor.getString(cursor.getColumnIndex(COLUMN_FAVOURITE_DESCRIPTION))
    ...
    fun addFavourite(context: Context, favourite: Favourites){
        val  values = ContentValues()
